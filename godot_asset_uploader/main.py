...
def auth_options(cmd):
    ...
    @option("--password", required_if_missing="token", hide_input=True, default=saved_auth("password"), callback=process_auth,
            help="Password to log in with. Will be prompted if not provided", cls=OptionRequiredIfMissing)
...
SHARED_PRIORITY_LIST = ["root", "readme", "no_prompt", "quiet", "token", "username", "password"]
