...
root_arg = cloup.argument(
    ...
    "or a VCS repository (currently, Git and Mercurial are supported). If not specified, it "
...
shared_auth_options = cloup.option_group(
    ...
    *SHORT_AUTH_OPTIONS,
    help="Authentication is allowed, but optional in dry-run mode, and required otherwise."
    "When authenticating, either --token or --username and --password are required, but not both.",
    constraint=Cond("dry_run", optional,
                    "token", accept_none,
                    else_=RequireNamed("username", "password")),
)
...
shared_update_options = make_options_decorator(
    ...
    cloup.option_group(
        ...
        option("--repo-provider", prompt=True, default=default_repo_provider, callback=process_repo_provider,
               type=click.Choice([x.name.lower() for x in vcs.RepoProvider], case_sensitive=False),
...
merged with another dict to provide missing values (this is the case for updates)"""
    ...
    return {
        ...
        "cost": cfg.licence,
        "download_provider": cfg.repo_provider.normalised,
